  ...
  python -m resume_tailor -r my_resume.md -j job_posting.txt -a -c -v
    ...
    if args.cover_letter:
        ...
        if args.format == "pdf":
            ...
            cl_pdf_bytes = markdown_to_pdf(cl_result["cover_letter"])
            cl_output_path.write_bytes(cl_pdf_bytes)
        elif args.format == "docx":
            cl_output_path = output_dir / "cover_letter.docx"
            cl_docx_bytes = markdown_to_docx(cl_result["cover_letter"])
            cl_output_path.write_bytes(cl_docx_bytes)
        else:
